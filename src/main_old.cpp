...
int main(int argc, char* args[]) {
    ...
    cl_mem clusterSumBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                             sizeof(float) * clusterSums.size(), clusterSums.data(), &contextResult);
    cl_mem clusterCountBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                               sizeof(int) * clusterCounts.size(), clusterCounts.data(), &contextResult);

    cl_kernel update_kernel = clCreateKernel(program, "updateClusters", &kernelResult);
    clSetKernelArg(cluster_kernel, 0, sizeof(cl_mem), &hsv_image);
    clSetKernelArg(update_kernel, 1, sizeof(cl_mem), &labelBuffer);
    clSetKernelArg(update_kernel, 2, sizeof(int), &width);
    clSetKernelArg(update_kernel, 3, sizeof(int), &height);
    clSetKernelArg(update_kernel, 4, sizeof(int), &numClusters);
    clSetKernelArg(update_kernel, 5, sizeof(cl_mem), &clusterSumBuffer);
    clSetKernelArg(update_kernel, 6, sizeof(cl_mem), &clusterCountBuffer);

    clEnqueueNDRangeKernel(commandQueue, update_kernel, 2, nullptr, globalWorkSize, nullptr, 0, nullptr, nullptr);
    clFinish(commandQueue);

    // Read back and compute new cluster centers
    clEnqueueReadBuffer(commandQueue, clusterSumBuffer, CL_TRUE, 0, sizeof(float) * clusterSums.size(), clusterSums.data(), 0, nullptr, nullptr);
    clEnqueueReadBuffer(commandQueue, clusterCountBuffer, CL_TRUE, 0, sizeof(int) * clusterCounts.size(), clusterCounts.data(), 0, nullptr, nullptr);

    for (int i = 0; i < numClusters; ++i) {
        int count = clusterCounts[i];
        if (count > 0) {
            clusterData[i * 5 + 0] = clusterSums[i * 5 + 0] / count; // H
            clusterData[i * 5 + 1] = clusterSums[i * 5 + 1] / count; // S
            clusterData[i * 5 + 2] = clusterSums[i * 5 + 2] / count; // V
            clusterData[i * 5 + 3] = clusterSums[i * 5 + 3] / count; // x
            clusterData[i * 5 + 4] = clusterSums[i * 5 + 4] / count; // y
        }
    }

    // Upload updated cluster data
    clEnqueueWriteBuffer(commandQueue, clusterBuffer, CL_TRUE, 0, sizeof(float) * clusterData.size(), clusterData.data(), 0, nullptr, nullptr);
    clFinish(commandQueue);
